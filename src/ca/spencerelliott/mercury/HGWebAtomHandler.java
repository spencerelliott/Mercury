...
public class HGWebAtomHandler implements AtomHandler {
	private HGWebParser parser;
	...
	private String URL_SUFFIX = "/atom-rss";
	private String URL_PREFIX = "";
	
	public HGWebAtomHandler() {
		buffer = new StringBuilder();
		parser = new HGWebParser();
	...
	public void characters(char[] ch, int start, int length)
			throws SAXException {
		buffer.append(new String(ch).substring(start, start+length));
	}
	...
	public void endElement(String uri, String localName, String qName)
		...
		if(new_entry) {
			...
			} else if((name.equals("pre") || name.equals("div")) && process_content) {
		...
		if(new_entry) {
			...
			if(localName.equals("link")) {
				if(atts.getIndex("href") >= 0) {
					...
					current_rev_id = "Rev. " + parser.parseRevisionID(current_link);
				}
